  ...
  const processHarFile = async (harData: any): Promise<NetworkRequest[]> => {
    ...
    try {
      ...
      const processedData = entries.map((entry: any) => {
        ...
        // Parse the ISO 8601 date string from HAR file
        let timestamp: number;
        try {
          // HAR files use ISO 8601 format (e.g., "2024-01-20T15:30:45.123Z")
    ...
    try {
      ...
      if (data.log && data.log.entries) {
        ...
        const processedData = data.map(entry => {
          ...
          return {
            ...entry,
            // Ensure timestamp exists in saved JSON files
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const processedData = data.map(entry => {
          ...
          return {
            ...entry,
            // Ensure timestamp exists in imported JSON files
            "9.timestamp": entry["9.timestamp"] || Date.now(),
            "parsed.cache-control": parsedCacheControl,
            "parsed.cache-used": determineCacheUsed(
              entry["8.fulfilledBy"], 
